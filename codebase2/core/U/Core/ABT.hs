...
cata :: Functor f =>
  ...
  Term f v a ->
  x
cata varAlg cycleAlg absAlg tmAlg =
  let go (Term fvs a out) = case out of
        Var x -> varAlg fvs a x
        Cycle x -> cycleAlg fvs a (go x)
        Abs arg body -> absAlg fvs a arg (go body)
        Tm inner -> tmAlg fvs a (fmap go inner)
  in go
