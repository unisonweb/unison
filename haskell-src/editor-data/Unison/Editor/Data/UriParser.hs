...
module Unison.Editor.Data.UriParser (repoPath) where
...
namespaceHashPath = do
  ...
  p <- P.optional $ do
    void $ C.char '.'
    P.sepBy1
      ((:) <$> C.satisfy Unison.Parser.Lexer.wordyIdStartChar
           <*> P.many (C.satisfy Unison.Parser.Lexer.wordyIdChar))
      (C.char '.')
  case p of
