...
--           Var _ -> []
--           Cycle body -> Unison.Codebase.V1.ABT.find p body
--           Abs _ body -> Unison.Codebase.V1.ABT.find p body
--           Tm body -> Foldable.concat (Unison.Codebase.V1.ABT.find p <$> body)

-- find' :: (Ord v, Foldable f, Functor f)
--   => (Term f v a -> Bool)
--   -> Term f v a
--   -> [Term f v a]
-- find' p = Unison.Codebase.V1.ABT.find (\t -> if p t then Found t else Continue)
