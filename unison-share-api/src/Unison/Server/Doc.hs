...
renderDoc pped terms typeOf eval types tm =
  ...
  where
    go = \case
      IOSource.Doc2Word txt -> pure $ Word txt
      IOSource.Doc2Code d -> Code <$> go d
      IOSource.Doc2CodeBlock lang d -> CodeBlock lang <$> go d
      IOSource.Doc2Bold d -> Bold <$> go d
      IOSource.Doc2Italic d -> Italic <$> go d
      IOSource.Doc2Strikethrough d -> Strikethrough <$> go d
      IOSource.Doc2Style s d -> Style s <$> go d
      IOSource.Doc2Anchor id d -> Anchor id <$> go d
      IOSource.Doc2Blockquote d -> Blockquote <$> go d
      IOSource.Doc2Blankline -> pure Blankline
      IOSource.Doc2Linebreak -> pure Linebreak
      IOSource.Doc2SectionBreak -> pure SectionBreak
      IOSource.Doc2Tooltip d1 d2 -> Tooltip <$> go d1 <*> go d2
      IOSource.Doc2Aside d -> Aside <$> go d
      IOSource.Doc2Callout Decls.OptionalNone' d -> Callout Nothing <$> go d
      IOSource.Doc2Callout (Decls.OptionalSome' icon) d -> Callout <$> (Just <$> go icon) <*> go d
      IOSource.Doc2Table rows -> Table <$> traverse r rows
        where
          r (Term.List' ds) = traverse go (toList ds)
          r _ = pure [Word "🆘 invalid table"]
      IOSource.Doc2Folded isFolded d d2 -> Folded isFolded <$> go d <*> go d2
      IOSource.Doc2Paragraph ds -> Paragraph <$> traverse go ds
      IOSource.Doc2BulletedList ds -> BulletedList <$> traverse go ds
      IOSource.Doc2NumberedList n ds -> NumberedList n <$> traverse go ds
      IOSource.Doc2Section title ds -> Section <$> go title <*> traverse go ds
      IOSource.Doc2NamedLink d1 d2 -> NamedLink <$> go d1 <*> go d2
      IOSource.Doc2Image d1 d2 Decls.OptionalNone' -> Image <$> go d1 <*> go d2 <*> pure Nothing
      IOSource.Doc2Image d1 d2 (Decls.OptionalSome' d) -> Image <$> go d1 <*> go d2 <*> (Just <$> go d)
      IOSource.Doc2Special sf -> Special <$> goSpecial sf
      IOSource.Doc2Join ds -> Join <$> traverse go ds
      IOSource.Doc2UntitledSection ds -> UntitledSection <$> traverse go ds
      IOSource.Doc2Column ds -> Column <$> traverse go ds
      IOSource.Doc2Group d -> Group <$> go d
      wat ->
    ...
    goSpecial = \case
      IOSource.Doc2SpecialFormFoldedSource (Term.List' es) -> FoldedSource <$> goSrc (toList es)
      -- Source [Either Link.Type Doc2.Term]
      IOSource.Doc2SpecialFormSource (Term.List' es) -> Source <$> goSrc (toList es)
      ...
      IOSource.Doc2SpecialFormExample n (IOSource.Doc2Example vs body) ->
        Example <$> source ex
        where
          ex = Term.lam' (ABT.annotation body) (drop (fromIntegral n) vs) body
      IOSource.Doc2SpecialFormExampleBlock n (IOSource.Doc2Example vs body) ->
        ExampleBlock <$> source ex
        where
          ex = Term.lam' (ABT.annotation body) (drop (fromIntegral n) vs) body

      -- Link (Either Link.Type Doc2.Term)
      IOSource.Doc2SpecialFormLink e ->
        let ppe = PPE.suffixifiedPPE pped
         ...
         in Link <$> case e of
              ...
              DD.EitherRight' (IOSource.Doc2Term (Term.Referent' r)) -> (pure . formatPretty . tm) r
              _ -> source e
      IOSource.Doc2SpecialFormSignature (Term.List' tms) ->
        let rs = [r | IOSource.Doc2Term (Term.Referent' r) <- toList tms]
         in goSignatures rs <&> \s -> Signature (map formatPretty s)
      -- SignatureInline Doc2.Term
      IOSource.Doc2SpecialFormSignatureInline (IOSource.Doc2Term (Term.Referent' r)) ->
        goSignatures [r] <&> \s -> SignatureInline (formatPretty (P.lines s))
      -- Eval Doc2.Term
      IOSource.Doc2SpecialFormEval (IOSource.Doc2Term tm) ->
        eval tm >>= \case
          Nothing -> Eval <$> source tm <*> pure evalErrMsg
          Just result -> Eval <$> source tm <*> source result
      -- EvalInline Doc2.Term
      IOSource.Doc2SpecialFormEvalInline (IOSource.Doc2Term tm) ->
        eval tm >>= \case
          Nothing -> EvalInline <$> source tm <*> pure evalErrMsg
          Just result -> EvalInline <$> source tm <*> source result
      -- Embed Video
      IOSource.Doc2SpecialFormEmbedVideo sources config ->
        pure $ Video sources' config'
        where
          sources' = [MediaSource url mimeType | IOSource.Doc2MediaSource (Term.Text' url) (maybeText -> mimeType) <- sources]
          config' = Map.fromList [(k, v) | Decls.TupleTerm' [Term.Text' k, Term.Text' v] <- config]
          maybeText (Term.App' _ (Term.Text' a)) = Just a
          maybeText _ = Nothing

      -- Embed FrontMatter
      IOSource.Doc2SpecialFormEmbedFrontMatter frontMatter ->
        pure $ FrontMatter frontMatter'
        where
          frontMatter' = List.multimap [(k, v) | Decls.TupleTerm' [Term.Text' k, Term.Text' v] <- frontMatter]

      -- Embed Any
      IOSource.Doc2SpecialFormEmbed (Term.App' _ any) ->
        source any <&> \p -> Embed ("{{ embed {{" <> p <> "}} }}")
      -- EmbedInline Any
      IOSource.Doc2SpecialFormEmbedInline any ->
    ...
    goSrc es = do
      let toRef (Term.Ref' r) = Set.singleton r
          ...
          go s1@(!seen, !acc) = \case
            ...
            DD.TupleTerm' [DD.EitherRight' (IOSource.Doc2Term tm), _anns] ->
              (seen <> toRef tm,) <$> acc'
              where
                acc' = case tm of
                  Term.Ref' r
