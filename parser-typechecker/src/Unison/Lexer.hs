...
lexer0 scope rem =
  ...
  where
    ...
    popLayout l pos rem = case matchKeyword' layoutCloseAndOpenKeywords rem of
      ...
      Just (kw, rem) ->
        let kw' = layoutCloseAndOpenKeywordMap kw l
         in case closes (openingKeyword kw') kw' l pos of
    ...
    pushLayout b l pos rem = span' isSpace rem $ \case
      ...
      (spaces, rem) ->
        let topcol = top l
            ...
            col' = column pos'
         in if b == "=" && col' <= topcol
              then -- force closing by introducing a fake col +1 layout
                popLayout0 ((b, col' + 1) : l) pos' rem
              else go ((b, col') : l) pos' rem
    ...
    go l pos rem = case rem of
      ...
      '?' : '\\' : c : rem ->
        case parseEscapeChar c of
          Just c ->
            let end = inc $ inc $ inc pos
             in Token (Character c) pos end : goWhitespace l end rem
          Nothing ->
      ...
      '[' : ':' : rem ->
        let end = inc . inc $ pos
         in Token (Open "[:") pos (inc . inc $ pos) : lexDoc l end rem
      ...
      '-' : '>' : rem@(c : _)
        | isSpace c || isAlphaNum c || Set.member c delimiters ->
          let end = incBy "->" pos
           in case topBlockName l of
                Just "match-with" ->
                  -- `->` opens a block when pattern-matching only
      ...
      '"' : rem -> case splitStringLit rem of
        Right (delta, lit, rem) ->
          let end = pos <> delta
           in Token (Textual lit) pos end : goWhitespace l end rem
    ...
    docBlob l pos rem blobStart acc = case rem of
      '@' : (hqToken (inc pos) -> Just (tok, rem)) ->
        let pos' = inc $ end tok
         in Token (Textual (reverse acc)) blobStart pos
              : tok
      ...
      '@' : (docType (inc pos) -> Just (typTok, pos', rem)) ->
        Token (Textual (reverse acc)) blobStart pos : case rem of
          (hqToken pos' -> Just (tok, rem)) ->
            let pos'' = inc (end tok)
             in typTok : tok : docBlob l pos'' rem pos' []
          _ -> recover l pos rem
    ...
    docType pos rem = case rem of
      ...
      '[' : (span (/= ']') -> (typ, ']' : (span isSpace -> (spaces, rem)))) ->
        ...
        let pos' = incBy typ . inc . incBy spaces . inc $ pos
         in -- the reserved token doesn't include the `[]` chars
...
shortHash s = case SH.fromString potentialHash of
  ...
  Just x -> Right (x, rem)
  where
    (potentialHash, rem) = break ((||) <$> isSpace <*> (== '`')) s
      ...
      case span symbolyIdChar s of
        ...
        (id, s') ->
          if Set.notMember id reservedOperators
            then go id s'
            else Left (InvalidSymbolyId id)
    (id, s') ->
      if Set.notMember id keywords && wordyIdStartChar (head id)
        then go id s'
        else Left (InvalidWordyId id)
  where
...
layoutCloseAndOpenKeywordMap ::
  String -> -- close-and-open keyword
  Layout -> -- layout
