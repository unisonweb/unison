        ...
        handleKindError ke = case ke of
          CycleDetected loc conflictedVar constraints ->
            let annotatedSrc =
                  showSourceMaybes
                    src
                    [ (,Type2) <$> rangeForAnnotated loc
                    ]
                (prettyVarKind, prettyVarKindConstraint) = prettyCyclicUVarKind env constraints conflictedVar (stylePretty Type1)
                theErrMsg =
                  Pr.hang
                        ...
                        [ annotatedSrc,
                          Pr.wrap
                            ( "The above application constrains the kind of"
                                <> stylePretty Type2 (prettyTyp conflictedVar)
                                <> "to be infinite, generated by the constraint"
                                <> prettyVarKindConstraint
                                <> "where"
                                <> stylePretty Type1 prettyVarKind
                                <> "is the kind of"
                                <> Pr.group (stylePretty Type2 (prettyTyp conflictedVar) <> ".")
                            )
                        ]
                    )
             in theErrMsg
