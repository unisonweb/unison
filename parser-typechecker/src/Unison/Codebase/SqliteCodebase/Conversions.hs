  ...
  where
    ...
    branch1to2 b =
      ...
      where
        ...
        doPatches :: Map NameSegment (V1.Branch.EditHash, m V1.Patch) -> Map NameSegment (V2.PatchHash, m V2.Branch.Patch)
        doPatches = Map.map (bimap edithash1to2 (fmap patch1to2))
...
branch2to1 branchCache lookupCT (V2.Branch.Branch v2terms v2types v2patches v2children) = do
  ...
  where
    v1patches = Map.map (bimap edithash2to1 (fmap patch2to1)) v2patches
