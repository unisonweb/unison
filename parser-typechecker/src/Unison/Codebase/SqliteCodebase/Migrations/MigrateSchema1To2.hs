...
migrateSchema1To2 conn codebase =
  Sqlite.runDB conn do
    Sqlite.withSavepoint "MIGRATESCHEMA12" \_rollback -> do
...
data Env m v a = Env {db :: Sqlite.Connection, codebase :: Codebase m v a}
...
migrateCausal :: MonadIO m => Sqlite.Connection -> CausalHashId -> StateT MigrationState m (Sync.TrySyncResult Entity)
migrateCausal conn oldCausalHashId = fmap (either id id) . runExceptT $ do
  ...
  newCausalHashId <- Sqlite.runDB conn (Q.saveCausalHash newCausalHash)
  let newCausal =
        DbCausal
...
migrateBranch :: MonadIO m => Sqlite.Connection -> ObjectId -> StateT MigrationState m (Sync.TrySyncResult Entity)
migrateBranch conn oldObjectId = fmap (either id id) . runExceptT $ do
  ...
  newHash <- Sqlite.runDB conn (Hashing.dbBranchHash newBranch)
  newHashId <- Sqlite.runDB conn (Q.saveBranchHash (BranchHash (Cv.hash1to2 newHash)))
  newObjectId <- Sqlite.runDB conn (Ops.saveBranchObject newHashId localBranchIds localBranch)
...
migratePatch conn oldObjectId = fmap (either id id) . runExceptT $ do
  ...
  let hashToHashId :: forall m. Sqlite.DB m => Hash -> m HashId
      hashToHashId h =
        fromMaybe (error $ "expected hashId for hash: " <> show h) <$> (Q.loadHashIdByHash (Cv.hash1to2 h))
  let hashToObjectId :: forall m. Sqlite.DB m => Hash -> m ObjectId
  ...
  newHash <- Sqlite.runDB conn (Hashing.dbPatchHash newPatchWithIds)
  newObjectId <- Sqlite.runDB conn (Ops.saveDbPatch (PatchHash (Cv.hash1to2 newHash)) (S.Patch.Format.Full localPatchIds localPatch))
  newHashId <- Sqlite.runDB conn (Q.expectHashIdByHash (Cv.hash1to2 newHash))
...
insertObjectMappingForHash conn oldHash newHash = do
  (oldObjectId, newHashId, newObjectId) <- Sqlite.runDB conn $ do
...
saveV2EmptyBranch :: Sqlite.DB m => m (BranchHashId, Hash)
