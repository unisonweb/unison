...
builtinMain :: (Var v, Monoid a) => a -> Type.Type v a
builtinMain a =
  let result = Var.named "result"
   in Type.forall a result (builtinMainWithResultType a (Type.var a result))

-- '{io2.IO, Exception} res
builtinMainWithResultType :: (Var v, Monoid a) => a -> Type.Type v a -> Type.Type v a
builtinMainWithResultType a res = Type.arrow a (Type.ref a DD.unitRef) io
  where
    io = Type.effect [Type.builtinIO a, DD.exceptionType a] res

-- [Result]
resultArr :: Ord v => a -> Type.Type v a
resultArr a = Type.app a (Type.ref a Type.listRef) (Type.ref a DD.testResultRef)

builtinResultArr :: (Ord v, Monoid a) => a -> Type.Type v a
builtinResultArr a = Type.effect [Type.builtinIO a, DD.exceptionType a] (resultArr a)

-- '{io2.IO} [Result]
builtinTest :: (Ord v, Monoid a) => a -> Type.Type v a
builtinTest a =
