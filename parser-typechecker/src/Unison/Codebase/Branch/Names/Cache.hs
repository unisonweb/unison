...
expectNamesForBranch codebase causalHash = do
  ...
  where
    ...
    buildNamesForBranch hashLen branch0 = do
      ...
      let allNamesPPED = PPED.fromNamesSuffixifiedByHash hashLen allNames
      let withoutTransitiveLibsPPED = PPED.fromNamesSuffixifiedByHash hashLen (withoutTransitiveLibsNames)
      BranchNames
