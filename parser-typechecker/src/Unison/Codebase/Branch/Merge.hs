...
merge'' lca mode b1@(Branch x) b2@(Branch y) =
  case mode of
    RegularMerge -> Branch <$> Causal.threeWayMerge' lca' combine x y
    SquashMerge  -> pure $ b1 `squashedOnto` b2 -- Causal.squashMerge' lca' (pure . discardHistory0) combine x y
 where
