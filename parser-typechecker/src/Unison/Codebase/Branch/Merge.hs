...
merge'' lca mode (Branch x) (Branch y) =
 ...
 where
  lca' c1 c2 = fmap history <$> lca (Branch c1) (Branch c2)
  combine :: Maybe (BranchSnapshot m) -> BranchSnapshot m -> BranchSnapshot m -> m (BranchSnapshot m)
  combine Nothing l r = merge0 lca mode l r
  combine (Just ca) l r = do
    dl <- diffSnapshot ca l
    dr <- diffSnapshot ca r
    ...
    pure $ branchSnapshot (terms head0) (types head0) children (edits head0)

  combineMissing ca k cur =
    case Map.lookup k (children ca) of
      Nothing -> pure $ Just cur
      Just old -> do
        nw <- merge'' lca mode (cons emptySnapshot old) cur
        if isEmptySnapshot $ head nw
        then pure Nothing
        else pure $ Just nw

  apply :: BranchSnapshot m -> BranchDiff -> m (BranchSnapshot m)
  apply b0 BranchDiff {..} = do
    patches <- sequenceA
      $ Map.differenceWith patchMerge (pure @m <$> edits b0) changedPatches
    let newPatches = makePatch <$> Map.difference changedPatches (edits b0)
        makePatch Patch.PatchDiff {..} =
          let p = Patch.Patch _addedTermEdits _addedTypeEdits
           in (H.accumulate' p, pure p)
    pure $ branchSnapshot (Star3.difference (terms b0) removedTerms <> addedTerms)
                   (Star3.difference (types b0) removedTypes <> addedTypes)
                   (children b0)
                   (patches <> newPatches)
...
merge0 :: forall m. Monad m => (Branch m -> Branch m -> m (Maybe (Branch m)))
                            -> MergeMode -> BranchSnapshot m -> BranchSnapshot m -> m (BranchSnapshot m)
merge0 lca mode b1 b2 = do
  c3 <- unionWithM (merge'' lca mode) (children b1) (children b2)
  e3 <- unionWithM g (edits b1) (edits b2)
  pure $ branchSnapshot (terms b1 <> terms b2)
                 (types b1 <> types b2)
                 c3
                 e3
  where
