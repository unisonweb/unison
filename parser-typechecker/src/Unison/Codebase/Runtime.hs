...
evaluateTerm' codeLookup cache ppe rt tm = do
  ...
  runnable <-
    if needsContainment rt
      then Codebase.makeSelfContained' (void codeLookup) uf
      else pure uf
  r <- evaluateWatches (void codeLookup) ppe cache rt runnable
  pure $ r <&> \(_,map) ->
...
evaluateTerm
  ...
  -> Term.Term v ()
  -> IO (Either Error (Term v))
evaluateTerm codeLookup ppe rt tm = evaluateTerm' codeLookup noCache ppe rt tm
