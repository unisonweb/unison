...
printNamespace :: ReadRepo -> Maybe ShortBranchHash -> Path 'Absolute -> Text
printNamespace repo sbh path =
  printReadRepo repo <> case sbh of
    Nothing -> if Path.isEmpty path then mempty
      else ":" <> Path.toText path
    Just sbh -> ":#" <> SBH.toText sbh <>
      if Path.isEmpty path then mempty
      else Path.toText path

printHead :: WriteRepo -> Path 'Absolute -> Text
printHead repo path =
  printWriteRepo repo
    <> if Path.isEmpty path then mempty 
                            else ":" <> Path.toText path

type ReadRemoteNamespace = (ReadRepo, Maybe ShortBranchHash, Path 'Absolute)
type WriteRemotePath = (WriteRepo, Path 'Absolute)
