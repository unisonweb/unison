...
-- todo: this is now going to look for a new branch head
...
fuzzyCompleteHashQualified :: Branch0 m -> String -> [Line.Completion]
...
autoCompleteHashQualified :: Branch0 m -> String -> [Line.Completion]
...
autoCompleteHashQualifiedTerm :: Branch0 m -> String -> [Line.Completion]
autoCompleteHashQualifiedTerm b (HQ.fromString -> query) =
  [ prettyCompletion (HQ.toString . SR.name $ sr, p)
  | (sr@(SR.Tm _), p) <- Find.prefixFindInBranch b query ]

autoCompleteHashQualifiedType :: Branch0 m -> String -> [Line.Completion]
