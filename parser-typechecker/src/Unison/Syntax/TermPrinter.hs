          ...
          notDoc go = do
            ...
            let -- This predicate controls which binary functions we render as infix
                  ...
                  Var' v ->
                    HQ.toName (HQ.unsafeFromVar v)
                      >>= operatorPrecedence
                        ...
                        . Name.lastSegment
                  _ -> Nothing
                -- Unwraps repeated binary infix application for a single operator.
                -- This is a special case that makes such expressions render nicely
                -- aligned between lines if the arguments are long.
                unBinaryAppsPred' ::
                  ( Term3 v PrintAnnotation,
                  ...
                  where
                    go t pred =
                      case unBinaryAppPred (t, pred) of
                        Just (f, x, y) ->
                          let -- We only chain together infix operators if they are
                              -- the same operator.
                              inChain g = isInfix g && (g == f)
                              l = unBinaryAppsPred' (x, inChain)
                              r = unBinaryAppsPred' (y, inChain)
                           in case (l, r) of
                                (Just (f, as, a), Just (g, bs, b))
                                  | f == g ->
                                      Just (f, as ++ (a : bs), b)
                                (Just (f, as, a), Nothing) ->
                                  Just (f, as ++ [a], y)
                                (Nothing, Just (f, bs, b)) ->
                                  Just (f, x : bs, b)
                                (Nothing, Nothing) -> Just (f, [x], y)
                                _ -> Nothing
                        Nothing -> Nothing

                -- Render repeated binary infix operator application as a single
                -- expression, with the operator in the middle and the arguments
                -- on either side. This is a special case that makes such expressions
                -- render nicely aligned between lines if the arguments are long.
                -- The output of this function is only used if the same operator
                ...
                binaryApps ::
                  ...
                  Pretty SyntaxText ->
                  m (Pretty SyntaxText)
                binaryApps f xs last =
                  do
                    psh <- join <$> traverse (r (InfixOp Lowest)) (take 1 xs)
                    pst <- join <$> traverse (r (InfixOp Highest)) (drop 1 xs)
                    let ps = psh <> pst
                        unbroken = PP.spaced (ps <> [last])
                        broken = PP.hang (head ps) . PP.column2 . psCols $ tail ps <> [last]
                    pure (unbroken `PP.orElse` broken)
                  where
                    fprec = termPrecedence f
                    ...
                    r p a =
                      let prec = if isBlock a then Top else fromMaybe p fprec
                       in sequenceA [pretty0 (ac prec Normal im doc) a, prettyF]
            ...
            case (term, binaryOpsPred) of
              ...
              app@(BinaryAppPred' f a b) -> do
                let prec = termPrecedence f
                    multi = unBinaryAppsPred' app
                case multi of
                  Just (f, args, lastArg)
                    | length args > 1 -> do
                        prettyLast <- pretty0 (ac (fromMaybe (InfixOp Lowest) prec) Normal im doc) lastArg
                        binaryApps f args prettyLast
                  _ -> do
