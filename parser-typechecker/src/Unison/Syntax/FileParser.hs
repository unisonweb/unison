                  ...
                  Just (ref, dataDecl) <-
                    ...
                    let kind = if decl.isClass then ClassRecord else TypeRecord
                     ...
                     in case kind of
                          ...
                          ClassRecord ->
                            -- Wrap each class getter with its
                            -- @=>@-bearing annotation so the
                            -- typechecker's @=>I@ rule binds the
                            -- getter's leading lambda variable as a
                            -- lexical given. The 'TypeRecord' branch
                            -- needs no extra wrapping — the
                            -- annotations baked in by
                            -- 'generateRecordAccessors' already
                            -- carry the rank-N field type.
                            map
