...
minimize
  ...
  -> Either [(v,a)] (Maybe (AnnotatedTerm' vt v a))
minimize (Term.LetRecNamedAnnotated' ann bs e) =
  case components (first snd <$> bs) of
    cs ->
      let varAnnotations = Map.fromList ((\((a, v), _) -> (v, a)) <$> bs)
          ...
          mklet cycle@((hdv, _) : _) e =
            Term.letRec (annotationFor hdv) (first annotatedVar <$> cycle) e
          mklet [] e = e
      in
...
minimize'
  :: Var v => AnnotatedTerm' vt v a -> Either [(v,a)] (AnnotatedTerm' vt v a)
