...
import Unison.Codebase.Editor.GitProtocol
...
-- $ git clone /srv/git/project.git[:treeish][:[#hash][.path]]
  [ scope "local-protocol" . tests . map parseAugmented $
    [ ("/srv/git/project.git",
      (GitRepo (LocalProtocol "/srv/git/project.git") Nothing, Nothing, Path.empty))
    , ("/srv/git/project.git:abc:#def.hij.klm",
      (GitRepo (LocalProtocol "/srv/git/project.git") (Just "abc"), sbh "def", path ["hij", "klm"]))
    , ("srv/git/project.git",
      (GitRepo (LocalProtocol "srv/git/project.git") Nothing, Nothing, Path.empty))
    , ("srv/git/project.git:abc:#def.hij.klm",
      (GitRepo (LocalProtocol "srv/git/project.git") (Just "abc"), sbh "def", path ["hij", "klm"]))
...
-- $ git clone file:///srv/git/project.git[:treeish][:[#hash][.path]] <- imagined
    ...
    [ ("file:///srv/git/project.git",
      (GitRepo (FileProtocol "/srv/git/project.git") Nothing, Nothing, Path.empty))
    , ("file:///srv/git/project.git:abc:#def.hij.klm",
      (GitRepo (FileProtocol "/srv/git/project.git") (Just "abc"), sbh "def", path ["hij", "klm"]))
    , ("file://srv/git/project.git",
      (GitRepo (FileProtocol "srv/git/project.git") Nothing, Nothing, Path.empty))
    , ("file://srv/git/project.git:abc:#def.hij.klm",
      (GitRepo (FileProtocol "srv/git/project.git") (Just "abc"), sbh "def", path ["hij", "klm"]))
...
-- $ git clone https://example.com/gitproject.git[:treeish][:[#hash][.path]] <- imagined
    ...
    [ ("https://example.com/git/project.git",
      (GitRepo (HttpsProtocol Nothing (HostInfo "example.com" Nothing) "/git/project.git") Nothing, Nothing, Path.empty))
    , ("https://user@example.com/git/project.git:abc:#def.hij.klm]",
      (GitRepo (HttpsProtocol (Just "user") (HostInfo "example.com" Nothing) "/git/project.git") (Just "abc"), sbh "def", path ["hij", "klm"]))
    ],
-- SSH Protocol
-- $ git clone ssh://[user@]server/project.git[:treeish][:[#hash][.path]]
    ...
    [ ("ssh://git@8.8.8.8:222/user/project.git",
      (GitRepo (SshProtocol (Just "git") (HostInfo "8.8.8.8" (Just "222")) "/user/project.git") Nothing, Nothing, Path.empty))
    , ("ssh://git@github.com/user/project.git:abc:#def.hij.klm",
      (GitRepo (SshProtocol (Just "git") (HostInfo "github.com" Nothing) "/user/project.git") (Just "abc"), sbh "def", path ["hij", "klm"]))
    ],
-- $ git clone [user@]server:project.git[:treeish][:[#hash][.path]]
    scope "scp-protocol" . tests . map parseAugmented $
    [ ("git@github.com:user/project.git",
      (GitRepo (ScpProtocol (Just "git") "github.com" "user/project.git") Nothing, Nothing, Path.empty))
    , ("github.com:user/project.git",
      (GitRepo (ScpProtocol Nothing "github.com" "user/project.git") Nothing, Nothing, Path.empty))
    , ("git@github.com:user/project.git:abc:#def.hij.klm",
      (GitRepo (ScpProtocol (Just "git") "github.com" "user/project.git") (Just "abc"), sbh "def", path ["hij", "klm"]))
    ]
  ]
