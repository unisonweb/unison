...
test = scope "path" . tests $
  ...
  , scope "parseHQ'Split'" . tests $
    [ let s = "foo.bar#34" in scope s . expect $
      parseHQ'Split' s == Right (relative ["foo"], HQ'.HashQualified (NameSegment "bar") (SH.unsafeFromText "#34"))
    , let s = "foo.bar.+" in scope s . expect $
      parseHQ'Split' s == Right (relative ["foo", "bar"], HQ'.NameOnly (NameSegment "+"))
    , let s = "#123" in scope s . expect $ isLeft $ parseHQ'Split' s
    ]
  ]
