...
-- derived client side using only the @Layout k@, so this type exists for
...
data Panel k t e = Panel {
  layout :: L.Layout k, -- ^ Specifies overall layout of the panel
  ...
  layouts :: M.Map k (L.Layout k) -- ^ For each cell, the prerendered layout
}
