...
debugTermReference :: TermReference -> Cli ()
debugTermReference ref = do
  ...
  case ref of
    Reference.DerivedId refId -> do
      Cli.runTransaction (Codebase.getTerm codebase refId) >>= \case
        ...
        Just term -> do
          Cli.respond $ DebugTerm (Right term)
    Reference.Builtin builtinTxt -> do
      Cli.respond $ DebugTerm (Left builtinTxt)
...
debugTerm :: HQ.HashQualified Name -> Cli ()
debugTerm hqName = do
  names <- Cli.currentNames
  let matches = Names.lookupHQTerm Names.IncludeSuffixes hqName names
  for_ matches \case
    Referent.Ref termReference -> debugTermReference termReference
