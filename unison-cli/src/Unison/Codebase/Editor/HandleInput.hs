...
-- | TODO: cache this in loop state
prettyPrintEnv :: (MonadState (LoopState.LoopState m v) n, MonadCommand n m i v) => n PPE.PrettyPrintEnv
prettyPrintEnv = do
...
loop = do
  ...
  let hqNameQuery q = eval $ HQNameQuery (Just currentPath') root' q
      ...
      basicPrettyPrintNames =
        Backend.prettyNamesForBranch root' (Backend.AllNames currentPath')
              ...
              goMany tms tys = do
                ...
                if null endangerments
                  then do
                    ...
                    ppe <- PPE.restrictTo currentPath' <$> prettyPrintEnv
                    respondNumbered $ CantDeleteDefinitions ppe endangerments
       in case input of
            ApiI -> eval API
                  ...
                  if null endangerments
                    then doDelete
                      ...
                      case insistence of
                        Force -> do
                          doDelete
                          respondNumbered $ DeletedDespiteDependents ppe endangerments
                        Try -> do
                          respondNumbered $ CantDeleteNamespace ppe endangerments
              where
                doDelete = do
...
handleShowDefinition outputLoc inputQuery = do
  ...
  when (not (null types && null terms)) do
    ...
    let ppe = Backend.getCurrentPrettyNames hqLength (PPE.RelativeTo currentPath') bias root'
...
doDisplay :: OutputLocation -> PPE.PrettyPrintEnv -> Term Symbol () -> Action' m Symbol ()
doDisplay outputLoc ppe tm = do
...
doShowTodoOutput patch scopePath = do
  ...
  let getPpe = do
        ppeFromLabeled (Patch.labeledDependencies patch)
    ...
    codebaseByMetadata = unlessError do
      ...
      lift case out of
        ...
        [(_name, ref, _tm)] -> do
          ...
          tm <- eval $ Evaluate1 ppe True tm
          case tm of
            Left e -> respond (EvaluationFailure e)
            Right tm -> doDisplay ConsoleLocation ppe (Term.unannotate tm)
        out -> do
          LoopState.numberedArgs .= fmap (HQ.toString . view _1) out
          respond $ ListOfLinks ppe out
...
suffixifiedPPE :: MonadCommand n m i v => NamesWithHistory -> n PPE.PrettyPrintEnv
suffixifiedPPE ns = do
  hashLen <- eval CodebaseHashLength
  pure $ PPE.fromNames hashLen Nothing PPE.Root Nothing PPE.Suffixify ns

fqnPPE :: MonadCommand n m i v => NamesWithHistory -> n PPE.PrettyPrintEnv
fqnPPE ns = do
  hashLen <- eval CodebaseHashLength
...
basicNames' :: (Functor m) => (Path.Absolute -> Backend.NameScoping) -> Action m i v (Names, Names)
basicNames' nameScoping = do
  root' <- use LoopState.root
  currentPath' <- use LoopState.currentPath
  let (parse, pretty, _local) = Backend.namesForBranch root' (nameScoping currentPath')
  pure (parse, pretty)
