...
type OptionFetcher = Codebase IO Symbol Ann -> PP.ProjectPathCtx -> Branch0 IO -> IO [Text]
...
multiResolver resolvers =
  let getOptions :: Codebase IO Symbol Ann -> PP.ProjectPathCtx -> Branch0 IO -> IO [Text]
...
projectBranchOptionsWithinCurrentProject codebase projCtx _searchBranch0 = do
  Codebase.runTransaction codebase (Q.loadAllProjectBranchesBeginningWith (projCtx ^. PP.ctxAsIds_ . PP.project_) Nothing)
    <&> fmap (into @Text . snd)
