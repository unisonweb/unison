...
checkCacheability cl ctx (r, sg) =
  ...
  where
    ...
    hasArrows abt = case ABT.out' abt of
      (ABT.Tm f) -> case f of
        Type.Arrow _ _ -> True
        -- 'ImplicitArrow' is still an arrow at the term level (the
        -- elaborator turns it into 'Arrow' before runtime, but for
        -- this caching predicate we treat it identically to
        -- 'Arrow').
