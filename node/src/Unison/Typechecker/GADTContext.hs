...
module Unison.Typechecker.GADTContext where -- (context, subtype, synthesizeClosed) where
...
data Principality = Principal | NotPrincipal deriving (Eq,Ord)

instance Show Principality where
  show Principal = "!"
  show NotPrincipal = ""

-- | Elements of an ordered algorithmic context
data Element
  = Universal ABT.V Kind               -- ^ `v` is universally quantified
  | Existential ABT.V Kind             -- ^ `v` existential and unsolved
  | Solved ABT.V Monotype Kind         -- ^ `v` is solved to some monotype
  | Ann ABT.V Type Principality        -- ^ `v` has type `a`, which may be quantified
  | Refined ABT.V Monotype
  | Marker ABT.V                       -- ^ used for scoping
  deriving Eq

instance Show Element where
  show (Universal v k) = show v ++ ":" ++ show k
  show (Existential v k) = "'"++show v ++ ":" ++ show k
  show (Solved v t k) = "'"++show v++" = "++show t ++ " : " ++ show k
  show (Refined v t) = show v++" == "++show t
  show (Ann v t p) = show v++" : "++show t++show p
  show (Marker v) = "|"++show v++"|"

-- | Kind-ignoring comparison of elements
(===) :: Element -> Element -> Bool
Existential v _ === Existential v2 _ | v == v2 = True
Universal v _ === Universal v2 _ | v == v2 = True
...
extend e c@(Context ctx) = Context ((e,i'):ctx) where
  ...
  addInfo e (Info es us vs ok) = case e of
    ...
    Universal v _ -> Info es (Set.insert v us) (Set.insert v vs) (ok && Set.notMember v us)
    -- EvarCtx - ensure no duplicates, and that this existential is not solved earlier in context
    Existential v _ -> Info (Set.insert v es) us (Set.insert v vs) (ok && Set.notMember v es)
    -- SolvedEvarCtx - ensure `v` is fresh, and the solution is well-formed wrt the context
    Solved v sa _ -> Info (Set.insert v es) us (Set.insert v vs) (ok && Set.notMember v es
                                                                      && wellformedType c (Type.getPolytype sa))
    -- VarCtx - ensure `v` is fresh, and annotation is well-formed wrt the context
    Ann v t _ -> Info es us (Set.insert v vs) (ok && Set.notMember v vs && wellformedType c t)
...
extendUniversal :: ABT.V -> Kind -> Context -> (ABT.V, Context)
extendUniversal v k ctx = case fresh v ctx of
  v -> (v, extend (Universal v k) ctx)

-- | Extend this `Context` with a marker variable, guaranteed to be fresh
extendMarker :: ABT.V -> Kind -> Context -> (ABT.V, Context)
extendMarker v k ctx = case fresh v ctx of
  v -> (v, ctx `append` context [Marker v, Existential v k])
...
ordered ctx v v2 = Set.member v (existentials (retract' (Existential v2 Kind.Star) ctx))

-- | Infer the kind of a type variable, `v`, based on how it is used in `body`
inferKind :: ABT.V -> Type -> Maybe Kind
inferKind v body = error "todo"
...
wellformedType c t = wellformed c && case t of
  ...
  Type.Forall' v t ->
    let (v',ctx2) = extendUniversal v Kind.Star c
...
bindings (Context ctx) = [(v,a) | (Ann v a _,_) <- ctx]

refinements :: Context -> [(ABT.V, Monotype)]
refinements (Context ctx) = [(v,a) | (Refined v a,_) <- ctx]
...
solve :: Context -> ABT.V -> Monotype -> Kind -> Maybe Context
solve ctx v t k
  | wellformedType ctxL (Type.getPolytype t) = Just ctx'
  | otherwise                                = Nothing
  where (ctxL,ctxR) = breakAt (Existential v k) ctx
        ctx' = ctxL `append` context [Solved v t k] `append` ctxR

-- | Replace any existentials with their solution in the context
-- and also apply any type refinements in the context
apply :: Context -> Type -> Type
apply ctx t = case t of
  Type.Universal' v -> maybe t Type.getPolytype (lookup v (refinements ctx))
  Type.Lit' _ -> t
  Type.Existential' v ->
  ...
  Type.Plus' t1 t2 -> Type.plus (apply ctx t1) (apply ctx t2)
  Type.Times' t1 t2 -> Type.times (apply ctx t1) (apply ctx t2)
  Type.Constrain' v c -> Type.constrain (apply ctx v) c
  Type.Forall' v t' -> Type.forall v (apply ctx t')
  Type.Exists' v t' -> Type.exists v (apply ctx t')
  Type.Implies' p t' -> Type.implies (fmap (apply ctx) p) (apply ctx t')
  Type.And' t' p -> Type.and' (apply ctx t') (fmap (apply ctx) p)
