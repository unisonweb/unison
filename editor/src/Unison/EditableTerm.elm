...
model term =
  let
    refresh : (Reference -> Metadata) -> Bool -> Trie Path.E Term -> Out -> Out
    refresh md raw evals s = case s.availableWidth of
    ...
    next : (Reference -> Metadata) -> Bool -> Trie Path.E Term -> Out -> Event
        -> Maybe (Moore Event Out)
    next md raw evals s e = case e of
      Metadata md ->
        let s' = refresh md raw evals s
        in Just <| Moore s' (next md raw evals s')
      AvailableWidth w ->
        let s' = refresh md raw evals { s | availableWidth <- Just w }
        in Just <| Moore s' (next md raw evals s')
      ToggleRaw ->
