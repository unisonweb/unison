...
//
//import org.unisonweb.{Lib2, Term, compilation2}
...
//  val N = 1e4
//
//  abstract class EvalLambda {
//    def apply(x0: U)(x0b: B): U
//    def apply(x1: U, x0: U)(x1b: B, x0b: B): U
//    def apply(xs: U*)(xbs: B*): U
//  }
//
//  def compileLambda(e: Term, r: R): EvalLambda = {
//    val cfn = compilation2.compile(Lib2.builtins)(e, isTail=true)
//    val us = new Array[U](1024)
//    val bs = new Array[B](1024)
//    cfn match {
//      case Return(f@Lambda(arity, body, _)) =>
//        new EvalLambda {
//          def apply(x0: U)(x0b: B): U =
//            eval(body, r, null, StackPtr.empty, us, U0, x0, bs, null, x0b)
//
//          def apply(x1: U, x0: U)(x1b: B, x0b: B): U =
//            eval(body, r, null, StackPtr.empty, us, x1, x0, bs, x1b, x0b)
//
//          def apply(xs: U*)(xbs: B*): U = {
//            assert(xs.length == xbs.length)
//            assert(xs.length > 2)
//            val stackArgCount = 0 min (xs.length - K)
//
//            @annotation.tailrec def copyStackArgs(i: Int): Unit =
//              if (i < stackArgCount) {
//                us(i) = xs(i)
//                bs(i) = xbs(i)
//                copyStackArgs(i+1)
//              }
//            copyStackArgs(0)
//            eval(body, r, null, StackPtr.empty.increment(stackArgCount),
//                 us, xs(xs.length-2), xs(xs.length-1),
//                 bs, xbs(xs.length-2), xbs(xs.length-1))
//          }
//        }
//      case fn => sys.error(s"what to do about ${fn}")
//
//    }
//
//  }
...
//      )('sum1toN)
//      val r = Result()
//      val evalLambda = compileLambda(sum1toN, r)
//      profile("unison loop") {
//        evalLambda((N + math.random).floor)(null).toLong
//      }
//    },
//    {
...
//        try return step(step, x0, x0b, x1, x1b, r)
//        catch { case _: SelfTC.type =>  }
//        loop(r.x0, r.x0b, r.x1, r.x1b, r)
//      }
//
//      val sum1toN = new Lambda {
//        def apply(rec: Lambda, n0: D, x0b: V, acc0: D, x1b: V, r: R): D =
