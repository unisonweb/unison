...
`branch foo`       forks the current project branch to a new
                   branch `foo`
`branch /bar foo`  forks the branch `bar` of the current project
                   to a new branch `foo`
`branch .bar foo`  forks the path `.bar` of the current project
                   to a new branch `foo`
...
`clone /main /main2`                      creates the branch
                                          ...
                                          remote (see
                                          `help-topics remotes`)
`clone /main my-fork/`                    creates `my-fork/main`
...
`delete.branch foo/bar`  deletes the branch `bar` in the project
                         `foo`
`delete.branch /bar`     deletes the branch `bar` in the current
                         project
...
`deprecated.cd foo.bar`   descends into foo.bar from the current
                          namespace.
...
`deprecated.cd ..`        moves to the parent of the current
                          namespace. E.g. moves from '.cat.dog'
                          to '.cat'
...
docs.to-html
`docs.to-html .path.to.ns doc-dir`                   Render all
                                                     docs
                                                     contained
                                                     within the
                                                     namespace
                                                     ...
                                                     UCM was run
                                                     from.
`docs.to-html project0/branch0:a.path /tmp/doc-dir`  Renders all
                                                     docs
                                                     anywhere in
                                                     the
                                                     namespace
...
`find foo bar`                   lists all definitions with a
                                 name similar to 'foo' or 'bar'
                                 in the current namespace
                                 (excluding those under 'lib').
...
`fork src dest`                                      creates the
                                                     namespace
                                                     `dest` as a
                                                     copy of
                                                     `src`.
`fork project0/branch0:a.path project1/branch1:foo`  creates the
                                                     namespace
                                                     `foo` in
                                                     `branch1`
                                                     of
                                                     ...
                                                     in
                                                     `project0/branch0`.
`fork srcproject/srcbranch dest`                     creates the
                                                     namespace
                                                     `dest` as a
                                                     copy of the
                                                     branch
...
`history #9dndk3kbsk13nbpeu`  Shows the history of the namespace
                              with the given hash. The full hash
                              must be provided.
...
`lib.install @unison/base/releases/latest`  installs the latest
                                            release of
                                            `@unison/base`
`lib.install @unison/base/releases/3.0.0`   installs version
                                            3.0.0 of
                                            `@unison/base`
`lib.install @unison/base/topic`            installs the `topic`
                                            branch of
                                            `@unison/base`
...
names.global
`names.global foo` shows the hash and all known names for `foo`.
...
`pull @unison/base/main`                merges the branch `main`
                                        of the Unison Share
                                        hosted project
                                        `@unison/base` into the
                                        current branch
`pull @unison/base/main my-base/topic`  merges the branch `main`
                                        of the Unison Share
                                        hosted project
                                        `@unison/base` into the
                                        branch `topic` of the
...
into a local branch without including the remote's history. This
usually results in smaller codebase sizes.
...
`push`                   publishes the current namespace or
                         branch. Remote mappings for namespaces
                         are configured in your `.unisonConfig`
                         at the key `RemoteMappings.<namespace>`
                         where `<namespace>` is the current
                         namespace. Remote mappings for branches
                         default to the branch that you cloned
                         from or pushed to initially. Otherwise,
                         it is pushed to
...
`push.create remote`        publishes the current namespace into
                            the empty remote namespace `remote`
`push.create`               publishes the current namespace into
                            the remote namespace configured in
                            your `.unisonConfig` at the key
                            `RemoteMappings.<namespace>` where
                            `<namespace>` is the current
                            namespace, then publishes the
                            current namespace to that location.
...
`reset #pvfd222s8n`         reset the current namespace to the
                            causal `#pvfd222s8n`
`reset foo`                 reset the current namespace to that
                            of the `foo` namespace.
`reset foo bar`             reset the namespace `bar` to that of
                            the `foo` namespace.
...
function that immediately returns a `@rewrite` block. It can be
in the codebase or scratch file. An example:

    rule1 x = @rewrite term x + 1 ==> Nat.increment x

Here, `x` will stand in for any expression wherever this rewrite
is applied, so this rule will match `(42+10+11) + 1` and replace
it with `Nat.increment (42+10+11)`.
...
run
`run mymain args...`  Runs `!mymain`, where `mymain` is searched
                      for in the most recent typechecked file,
                      or in the codebase. Any provided arguments
                      will be passed as program arguments as
                      though they were provided at the command
                      line when running mymain as an executable.

run.native
`run.native main args`  Executes !main using native compilation
                        via scheme.
...
`switch foo/bar`  switches to the branch `bar` in the project
                  `foo`
`switch foo/`     switches to the last branch you visited in the
                  project `foo`
...
including conflicted names, dependencies with missing names, and
...
patch.`update.old`                  adds all definitions in the
                              .u file, noting replacements in
...
update.old.nopatch
`update.old.nopatch` works like `update.old`, except it doesn't
add a patch entry for any updates. Use this when you want to
make changes to definitions without pushing those changes to
dependents beyond your codebase. An example is when updating
docs, or when updating a term you just added.`update.old.nopatch`          updates
...
needs update         A definition with the same name as an
                     existing definition. Doing `update` instead
                     of `add` will turn this failure into a
                     successful update.
...
ctor/term collision  A type defined in the file has a
                     ...
                     constructor before trying again to `add` or
                     `update`.
...
extra dependency     This definition was added because it was a
                     dependency of a definition explicitly
                     selected.
...
Relative names are converted to absolute names by prepending the
current namespace. For example, if your Unison prompt reads:
...
and you can refer to x by its absolute name .foo.bar.x elsewhere
...
A project is a versioned collection of code that can be edited,
published, and depended on other projects. Unison projects are
analogous to Git repositories.
...
Local projects may be associated with at most one remote project
on Unison Share. When this relationship is established, it
becomes the default argument for a number of share commands. For
example, running `push` or `pull` in a project with no arguments
will push to or pull from the associated remote, if it exists.

This association is created automatically on when a project is
created by `clone`. If the project was created locally then the
relationship will be established on the first `push`.
...
these expressions are pure and always yield the same result when
evaluated, there's no need to run them more than once!
