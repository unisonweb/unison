...
.> cd testio.old
  ...
  ☝️  The namespace .testio.old is empty.
...
```ucm
  ...
  change:
    ...
    ⍟ These new definitions are ok to `add`:
      ...
      type io.Mode
      foo           : ∀ (). () ->{IO} ()
      io1.printLine : Text ->{IO1} ()
      io1.putText   : Handle -> Text ->{IO1} ()
      io1.rethrow   : Either Error a ->{IO1} a
    
    x These definitions would fail on `add` or `update`:
    
      Reason
      needs alias   type Either a b                   (existing name: .builtin.Either)
      needs alias   unique type IsTest                (existing name: .builtin.IsTest)
      needs alias   type Optional a                   (existing name: .builtin.Optional)
      needs alias   unique type io.BufferMode         (existing name: .builtin.io.BufferMode)
      needs alias   unique type io.EpochTime          (existing name: .builtin.io.EpochTime)
      needs alias   type io.Error                     (existing name: .builtin.io.Error)
      needs alias   unique type io.ErrorDescription   (existing name: .builtin.io.ErrorDescription)
      needs alias   unique type io.ErrorLocation      (existing name: .builtin.io.ErrorLocation)
      needs alias   unique type io.ErrorType          (existing name: .builtin.io.ErrorType)
      needs alias   unique type io.FilePath           (existing name: .builtin.io.FilePath)
      needs alias   unique type io.HostName           (existing name: .builtin.io.HostName)
      needs alias   unique type io.SeekMode           (existing name: .builtin.io.SeekMode)
      needs alias   unique type io.ServiceName        (existing name: .builtin.io.ServiceName)
      needs alias   unique type io.Socket             (existing name: .builtin.io.Socket)
      needs alias   unique type io.ThreadId           (existing name: .builtin.io.ThreadId)
      needs alias   unique type io1.Handle            (existing name: .builtin.io.Handle)
      needs alias   io1.stderr, .builtin.io.stderr   : Handle
      needs alias   io1.stdin, .builtin.io.stdin     : Handle
      needs alias   io1.stdout, .builtin.io.stdout   : Handle
...
```ucm
  ☝️  The namespace .testio1 is empty.
