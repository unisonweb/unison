...
```ucm
  ...
  `>`)... Ctrl+C cancels.
    ...
    7 | > smallCompressed = Bytes.size (zlib.compress (Value.serialize.small (Value.value xs)))
          ⧩
          1292
  
    8 | > bigCompressed   = Bytes.size (zlib.compress (Value.serialize (Value.value xs)))
          ⧩
          1532
