...
serverThread : MVar Text -> Text -> '{io2.IO} (Either Failure ())
serverThread mv toSend = 
  go = 'let
    sock = toException (serverSocket (Some "127.0.0.1") "0")
    port = toException (socketPort sock)
    toException (MVar.put mv (toText port))
...
clientThread : MVar Text -> MVar Text -> '{io2.IO} (Either Failure ())
clientThread portVar resultVar = 
  go : '{io2.IO, Exception Failure}()
  go = 'let
    port = toException (read portVar)
    sock = toException (clientSocket "127.0.0.1" port)
