...
data Pattern loc
  ...
  | Record loc (Map Text (Pattern loc))
...
updateDependencies tms p = case p of
  ...
  Record loc ps ->
    Record loc (updateDependencies tms <$> ps)
...
hasSubpattern needle haystack = needle == haystack || go haystack
  where
    ...
    go (Record _ ps) = any (hasSubpattern needle) (Map.elems ps)
...
instance Show (Pattern loc) where
  ...
  show (Record _ ps) =
    "Record " <> intercalate ", " (fmap (\(k, v) -> show k <> ": " <> show v) $ Map.toList ps)
...
foldMap' f p = case p of
  ...
  Record _ ps -> f p <> foldMap (foldMap' f) (Map.elems ps)
