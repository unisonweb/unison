...
suffixedTypeName :: Int -> Maybe Name -> Reference -> NamesWithHistory -> [HQ'.HashQualified Name]
suffixedTermName :: Int -> Maybe Name -> Referent -> NamesWithHistory -> [HQ'.HashQualified Name]
(suffixedTermName, suffixedTypeName) =
  ...
  where
    suffixedName fallback getRel hq' hashLen mayBias r ns@(getRel -> rel) =
      if R.memberRan r rel
        then go $ toList (R.lookupRan r rel)
        else sort $ Set.toList (fallback hashLen r ns)
      where
        -- Orders names, using these criteria, in this order:
        -- 1. NameOnly comes before HashQualified,
        -- 2. Names with a longer common-prefix with the 'bias' name are preferred if one is provided
        -- 3. Names with shorter fully-qualified names (in terms of segment count) come before longer ones
        -- 4. Shorter _suffixified_ names (in terms of segment count) come before longer ones
        -- 5. If same on all other attributes, compare alphabetically
        go :: [Name] -> [HQ'.HashQualified Name]
        go fqns = map (view _6) . sort $ map f fqns
          where
            f fqn =
